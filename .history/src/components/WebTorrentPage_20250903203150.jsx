  ...
  useEffect(() => {
    ...
    const initializeWebTorrent = async () => {
      try {
        ...
        setIsDesktop(electronDetected);

        if (electronDetected) {
          // Use enhanced desktop client
          const desktopWebTorrent = new DesktopWebTorrent();
          await desktopWebTorrent.init();
          setDesktopClient(desktopWebTorrent);
          setClient(desktopWebTorrent.client);
          setIsLoading(false);
          showNotification(
  ...
  const createTorrent = async () => {
    ...
    try {
      ...
      if (isDesktop && desktopClient) {
        torrent = await desktopClient.createTorrent(selectedFiles);
        showNotification("Desktop torrent created with enhanced connectivity!");
      } else {
        torrent = await new Promise((resolve) => {
          client.seed(selectedFiles, resolve);
        });
